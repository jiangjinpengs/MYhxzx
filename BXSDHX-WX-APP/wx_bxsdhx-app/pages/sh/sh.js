...
Page({
   ...
   */
  Release: function() { //发布
    if (this.data.inputtitle != "" && this.data.inputcomtent != "") {
      wx.showLoading({
        title: '发布中···',
      })
      const das = {
        title: this.data.inputtitle, //标题
        comtent: this.data.inputcomtent, //内容
        name: this.data.user.data._id, //谁发布的//
        model: "Commonsenseoflife", //数据库对象
        apiname: "Datamanipulation", //接口门牌
        functions: "increase",
        types: "yes"
  ...
  inputcomtent: function(e) {
    this.setData({
      inputcomtent: e.detail.value
    })
  },
  noRelease: function() { //取消发布
    this.setData({
      stylego: "none"
    })
  },
